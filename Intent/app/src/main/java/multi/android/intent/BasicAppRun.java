...
public class BasicAppRun extends AppCompatActivity {
    ...
    //승인받을 권한의 목록
    //권한이 String이라서 String 배열로 관리한다.
    //리소스는 int라서 추후 int로 관리한다.
    ...
    public void runGoogleMap(View v){
        Uri uri = Uri.parse("geo:37.501580,127.039599");
    ...
    public void rundial(View v){
        Uri uri = Uri.parse("tel:01074965575");
    ...
    public void runCallPhone(View v){
        ...
        if(chk==PackageManager.PERMISSION_GRANTED){//허가가 된 상태
            Log.d("tel","성공");
            intent = new Intent(Intent.ACTION_CALL,
                    Uri.parse("tel:01029093170"));
        }else {
            Log.d("tel","실패");
            return;
        }
